...
| R-006 | This repository is public and GitHub Pages serves every root-level file. All governance documentation added here is world-readable, and `REPOSITORY_HANDOFF_CONFIG.md` contained absolute local filesystem paths. | Medium | High | Project owner chose "remove" 2026-08-19: repository root and the Snapshot Destination by Machine table (paths + `ComputerName` detection) moved to AntBrainOS vault `03_PROJECTS/Active/K_to_Career_Website/REPOSITORY_HANDOFF_CONFIG_LOCAL.md`; repo file now points there | Project owner | Resolved 2026-08-19 |
